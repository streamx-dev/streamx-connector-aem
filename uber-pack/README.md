...
This directory provides an uber [Apache Jackrabbit FileVault](https://jackrabbit.apache.org/filevault/index.html) package with the Adobe Experience Manager (AEM) Connector for [StreamX](https://www.streamx.dev/StreamX) (*"Uber Package with AEM Connector"*). The term _uber_ signifies that the package includes all necessary dependencies and OSGi configurations required for the AEM Connector to function within AEM.
...
The Uber Package with AEM Connector is compatible exclusively with AEM as a Cloud Service.
...
| Factory PID/PID                                                | Property name in OSGi configuration | Name of corresponding environment variable | Default value set by the Uber Package with AEM Connector |
|----------------------------------------------------------------|-------------------------------------|--------------------------------------------|----------------------------------------------------------|
...
<dependency>
    ...
    <artifactId>streamx-connector-aem-uber-pack-all</artifactId>
    <version>0.0.7</version>
    <type>zip</type>
</dependency>
```

> **Warning:** Installing the Uber Package with AEM Connector alone is insufficient for the AEM Connector to function correctly. See [Mandatory Configuration Change](#mandatory-configuration-change) for necessary configuration changes.
...
3. Add the Uber Package with AEM Connector as a dependency in the `dependencies` section of `all/pom.xml`:
...
4. Configure the `filevault-package-maven-plugin` in `all/pom.xml` to embed the Uber Package with AEM Connector:
...
5. Deploy the `all` deployment artifact to a running AEM instance. Once deployed, the Uber Package with AEM Connector will be installed.
...
1. To deploy the Uber Package with AEM Connector to a running AEM Author instance in the Rapid Development Environment (RDE), first build the package:

    ```bash
    mvn clean package
    ```

   This will create an artifact at `uber-pack.all/target/streamx-connector-aem-uber-pack-all-0.0.7`.
...
2. Deploy the artifact to an AEM Author instance in RDE using [AIO CLI](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/local-development-environment-set-up/development-tools#aio-cli). Replace `<your-programId>` and `<your-environmentId>` with the relevant values:
    ...
    ```bash
    aio aem:rde:install uber-pack.all/target/streamx-connector-aem-uber-pack-all-0.0.7 --programId <your-programId> --environmentId <your-environmentId>
    ```
