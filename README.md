...
The parameter is named `isContentPushedToStreamx` and has the value `"true"`.
...
When the `isContentPushedToStreamx` flag is set to `"true"`, you can render a special HTL expression such as:
...
<sly data-sly-test="${model.contentPushedToStreamx && properties.fragmentVariationPath != null}">
        {{#include src="${properties.fragmentVariationPath}.html"}}
    </sly>
</sly>

<sly data-sly-test="${!model.contentPushedToStreamx}">
...
public class StreamxExperienceFragmentModel {
  ...
  @Getter
  private final boolean isContentPushedToStreamx;

  @Inject
  public StreamxExperienceFragmentModel(SlingHttpServletRequest request) {
    isContentPushedToStreamx = Optional
        .ofNullable(request.getRequestParameter("isContentPushedToStreamx"))
